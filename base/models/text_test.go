...
func TestMessageTemplate_Decode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			sd:   &MessageTemplate{},
			args: args{
				value: `{"waitAgent":"Esperando un agente","welcomeTemplate":"Hola soy %s y necesito ayuda","context":"Contexto","DescriptionCase":"caso levantado por el Bot","uploadImageError":"Imagen no enviada","uploadImageSuccess":"**El usuario adjunto una imagen al caso**","uploadFileError":"Archivo no enviado","uploadFileSuccess":"**El usuario adjunto un archivo al caso**","queuePosition":"Posición en la cola","waitTime":"Tiempo de espera","clientLabel":"Cliente","botLabel":"Bot"}`,
			},
			wantErr: false,
			want: &MessageTemplate{
				...
				UploadFileError:    "Archivo no enviado",
				UploadFileSuccess:  "**El usuario adjunto un archivo al caso**",
				ClientLabel:        "Cliente",
				BotLabel:           "Bot",
			},
		},
